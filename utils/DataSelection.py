...
def get_base_index(react_desc,threshold,init_idx=None,rand_seed=None):
    ...
    base_index = [init_idx]
    diff_deg = np.arccos(np.array([cosine_similarity(react_desc[init_idx],
...
def get_selected_index(point_desc,pot_sel_desc,method='euclidean',threshold=2):
    ...
    if method == 'euclidean':
        dist = np.linalg.norm(point_desc - pot_sel_desc,axis=1)
        sel_idx = np.where(dist < threshold)[0]
    elif method == 'cosine':
        angle = np.arccos(np.array([cosine_similarity(point_desc,
